...
#include "pipe.def"
    ...
    fetch_sgn.bubble_for_decode = (de.instFlags & F_CTRL) && (de.instFlags & F_UNCOND);
}
...
void do_ex()
{
  ...
  decode_sgn.bubble_for_execute = em.needJump && (buf.de.mem_ctrl.Branch != BRH_UNCOND);
}
...
void forwarding() {
  // Reset stall signal
  fetch_sgn.stall_for_execute = FALSE;
  // Memory-writeback forward to decode stage
  bool_t m_write_reg = mw.wb_ctrl.RegWrite && mw.reg_dest != 0;
  sword_t m_forword_data = mw.wb_ctrl.MemtoReg ? mw.memread_res : mw.alu_res;
  de.reg1_data = (m_write_reg && (mw.reg_dest == de.oprand.in1)) ?
                     m_forword_data : de.reg1_data;
  de.reg2_data = (m_write_reg && (mw.reg_dest == de.oprand.in2)) ?
                     m_forword_data : de.reg2_data;
  de.reg3_data = (m_write_reg && (mw.reg_dest == de.oprand.in3)) ?
                     m_forword_data : de.reg3_data;

  // Execute-memory forward to decode stage
  bool_t e_write_reg = em.wb_ctrl.RegWrite && em.reg_dest != 0;
  bool_t e_request_stall = em.wb_ctrl.MemtoReg;
  sword_t e_forword_data = em.alu_res;
  if (!e_request_stall) {
    de.reg1_data = (e_write_reg && (em.reg_dest == de.oprand.in1)) ?
                       e_forword_data : de.reg1_data;
    de.reg2_data = (e_write_reg && (em.reg_dest == de.oprand.in2)) ?
                       e_forword_data : de.reg2_data;
    de.reg3_data = (e_write_reg && (em.reg_dest == de.oprand.in3)) ?
                       e_forword_data : de.reg3_data;
  } else {
    decode_sgn.bubble_for_execute = TRUE;
    fetch_sgn.stall_for_execute = TRUE;
  }
